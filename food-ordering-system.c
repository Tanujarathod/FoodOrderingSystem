...
//Structure Item which contains variable of name, rate and also node( for linked list)
struct Item
{
    char name[50];
    float rate;
...
//another structure to create another linked list 
struct FoodItem
{
    char username[20];
	int qty;
...
void autoCreateMenu()
{
    ...
    float rates[10] = {60,30,60,90,50,60,40,20,30,15};
    struct Item *newnode,*temp;
    for(i=0;i<10;i++)
    {
       newnode = (struct Item *)malloc(sizeof(struct Item));
